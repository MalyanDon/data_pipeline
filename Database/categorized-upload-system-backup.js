...
app.post('/api/upload/:category/:subcategory', upload.single('file'), async (req, res) => {
    try {
        ...
        console.log(`📤 Processing upload: ${category}/${subcategory}`);
        ...
        // Add metadata
        const date = processingDate || new Date().toISOString().split('T')[0];
        ...
        const collectionName = `${category}.${date}.${subcategory}`;
...
function extractDateFromFilename(filename) {
    ...
    const datePatterns = [
        ...
        /(\d{1,2})[\/\-_](\d{1,2})[\/\-_](\d{4})/,
        // MM/DD/YYYY or MM-DD-YYYY or MM_DD_YYYY  
        /(\d{1,2})[\/\-_](\d{1,2})[\/\-_](\d{4})/,
...
<html><head><title>Raw Data Upload System - MongoDB</title><style>
...
<div class="container">
    <div class="header">
                 <h1 class="title">📁 Raw Data Upload System</h1>
         <p>Step 1: Upload and organize raw financial data into MongoDB by category</p>
    </div>
    ...
    <div class="upload-section">
        ...
        <div class="form-group">
            ...
            <input type="date" id="processing-date" style="width:100%;padding:12px;border:2px solid #e1e5e9;border-radius:8px" value="${new Date().toISOString().split('T')[0]}">
        </div>
...
async function uploadFile() {
    if (!selectedCategory || !selectedFile) {
        showMessage('Please select a category, subcategory, and file.', 'error');
        return;
    }
    ...
    try {
        const response = await fetch('/api/upload/' + selectedCategory + '/' + subcategory, {
            method: 'POST',
            body: formData
        });
        ...
        if (result.success) {
            showMessage(
                'File uploaded successfully! ' + result.details.recordCount + 
                ' records added to ' + result.details.category + ' (' + result.details.subcategory + ')', 
                'success'
            );
            resetForm();
        } else {
...
app.listen(PORT, () => {
    console.log(`📁 Categorized Upload System running at http://localhost:${PORT}`);
    console.log('📊 Supporting categories:');
    Object.keys(DATA_CATEGORIES).forEach(key => {
        console.log(`   ${DATA_CATEGORIES[key].icon} ${DATA_CATEGORIES[key].displayName}`);
    });
});
